...
class Z_network(nn.Module):
    def __init__(self):
        ...
        o_encoder1 = nn.Linear(O_DIM, 12)
        o_encoder2 = nn.Linear(12, 6)
        prior1 = nn.Linear(H_DIM+M_DIM*Kr, 2*Z_DIM)
        prior2 = nn.Linear(2*Z_DIM, 2*Z_DIM)
        prior3 = nn.Linear(2*Z_DIM, 2*Z_DIM)
        f_post1 = nn.Linear(6+A_DIM+1+H_DIM+M_DIM*Kr+2*Z_DIM, 2*Z_DIM)
        f_post2 = nn.Linear(2*Z_DIM, 2*Z_DIM)
        f_post3 = nn.Linear(2*Z_DIM, 2*Z_DIM)
